    ...
    for idx in range(1, 9):
        ...
        if idx == 1:
            result_df[f"infire_day_{idx}"] = (
                model.predict_proba(test[FEATURES])[:, 1] > 0.9
            ).astype(int)
        elif idx == 2:
            result_df[f"infire_day_{idx}"] = (
                model.predict_proba(test[FEATURES])[:, 1] > 0.9
            ...
            result_df[f"infire_day_{idx}"] = (
                model.predict_proba(test[FEATURES])[:, 1] > 0.575
            ).astype(int)
        elif idx == 4:
            result_df[f"infire_day_{idx}"] = (
                model.predict_proba(test[FEATURES])[:, 1] > 0.55
            ).astype(int)
        elif idx == 5:
            result_df[f"infire_day_{idx}"] = (
                model.predict_proba(test[FEATURES])[:, 1] > 0.35
            ).astype(int)
        elif idx == 6:
            result_df[f"infire_day_{idx}"] = (
                model.predict_proba(test[FEATURES])[:, 1] > 0.22
            ).astype(int)
        elif idx == 7:
            result_df[f"infire_day_{idx}"] = (
                model.predict_proba(test[FEATURES])[:, 1] > 0.13
            ).astype(int)
        elif idx == 8:
            result_df[f"infire_day_{idx}"] = (
                model.predict_proba(test[FEATURES])[:, 1] > 0.09
            ).astype(int)
        else:
            result_df[f"infire_day_{idx}"] = (
                model.predict_proba(test[FEATURES])[:, 1] > 0.04
            ).astype(int)

  
    logger.info("save output.csv")
    result_df.drop("infire_day", axis=1, errors="ignore").to_csv(
